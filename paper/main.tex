...
VirtualDJ cross-platform analysis (Table~\ref{tab:vdj-rb}) confirms these findings. Matching 22 tracks between Rekordbox and VirtualDJ by file path and duration confirms identical audio files. Among the 19 tracks with BPM values in both platforms: 6 (32\%) agree within 1 BPM (mean 0.15 BPM difference); 5 tracks (25\%) from the ETHEREAL AFTERHOURS playlist exhibit a consistent 1.19$\times$ ratio (Rekordbox stores 140.0--141.4 BPM, VirtualDJ stores 118.0--119.0 BPM), a distinct pattern from the Rekordbox 2$\times$ convention that suggests algorithmic half-tempo interpretation rather than a storage error; the remaining 9 tracks (45\%) show unexplained BPM disagreements without a clean ratio, concentrated in tracks at the 140--175 BPM stored range. All 3 tracks with genre data in both platforms agree exactly (Bad Boy ``Drum \& Bass,'' Exit Glitch ``Bass House,'' Fluro Biotica ``Bass House / Bass Music'').
...
The 190-track VirtualDJ export provides an additional cross-validation source. VirtualDJ stores BPM as a decimal fraction of the beat-grid unit; the raw value is multiplied by 282 to produce the reported BPM (e.g., raw=0.461538 yields 130.2 BPM). VirtualDJ field coverage differs markedly from Rekordbox and Serato: BPM coverage is 77.9\% (148/190), lower than Rekordbox (92.2\%) and comparable to Serato (84.8\%); key coverage is 99.5\% (189/190), the highest of any platform in this study; genre coverage is only 28.9\% (55/190), the lowest across all platforms; artist coverage is 96.8\% (152/157), comparable to Rekordbox and Serato.

Of the 22 tracks appearing in both Rekordbox and VirtualDJ (matched by title and artist), 19 have BPM values in both platforms, enabling direct tempo comparison. The disagreements fall into two distinct clusters:
...
We present the first cross-platform metadata quality analysis of a DJ software library using UDMS, a Unified DJ Metadata Schema. Our analysis of 636 Rekordbox tracks, 382 Serato tracks, 112 Engine DJ tracks, and 190 VirtualDJ tracks reveals that while audio-derived fields and musical key achieve high coverage (93--98\% across platforms), genre annotation, the field most critical for automated playlist generation, is incomplete for 31--69\% of tracks across platforms. Cross-platform interoperability analysis across 143 matched tracks between Rekordbox and Serato shows that BPM and key are reliably preserved (89.5\% and 100\% effective agreement), but genre agreement is only 79.1\% even when both platforms are tagged. A previously undocumented Rekordbox 2$\times$ BPM interpretation difference affects 27\% of tracks; we validate this interpretation difference against Engine DJ, which stores the canonical tempo convention. Cross-validation with VirtualDJ reveals a distinct half-tempo interpretation pattern (1.19$\times$ ratio) affecting six tracks from a single DJ playlist, demonstrating that BPM disagreement between platforms can also reflect algorithmic differences in tempo interpretation. Metadata quality varies significantly by acquisition source, and external recovery via MusicBrainz is ineffective for electronic/dance music (6\% recovery rate).
