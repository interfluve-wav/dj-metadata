...
%%% arXiv-Title: UDMS: A Unified Schema for Assessing Metadata Quality Across DJ Ecosystems
...
\title{UDMS: A Unified Schema for Assessing Metadata Quality Across {DJ} Ecosystems}
...
The VirtualDJ analysis reveals that key coverage is the Rekordbox export's blind spot: Rekordbox's \texttt{TonalityKey} field is 0\% populated in the analyzed XML export, while VirtualDJ achieves 97.5\% key coverage on the same underlying audio files. This cross-platform complementarity validates UDMS's role as a aggregator: no single platform provides complete metadata, but combining multiple platforms' exports covers more fields than any individual source.
