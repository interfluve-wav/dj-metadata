...
We address this gap with the first cross-platform metadata quality analysis of real-world DJ libraries. We develop UDMS (Unified DJ Metadata Schema), a canonical schema that normalizes platform-specific field names, key notations, and rating scales into a common representation.  Using UDMS, we analyze a 636-track Rekordbox library, a 382-track Serato library, a 112-track Engine DJ library, and a 157-track VirtualDJ library from the same DJ, enabling the first cross-platform DJ metadata interoperability study. By matching 143 tracks across Rekordbox and Serato, we quantify preservation rates across all UDMS fields and characterize a systematic 2$\times$ half-tempo BPM interpretation difference in Rekordbox for half-tempo electronic music genres. A preliminary analysis of 22 matched Rekordbox--VirtualDJ tracks provides initial evidence of a distinct 1.19$\times$ algorithmic half-tempo interpretation pattern, though this requires validation on a larger matched sample.
...
\begin{itemize}
  ...
  \item The first cross-platform DJ metadata interoperability analysis: using 143 tracks matched between Rekordbox and Serato, we quantify preservation rates across all UDMS fields, characterize a 2$\times$ half-tempo BPM interpretation difference in Rekordbox that affects a substantial fraction of half-tempo tracks, and present preliminary evidence from 22 matched Rekordbox--VirtualDJ tracks suggesting a distinct 1.19$\times$ algorithmic half-tempo interpretation pattern in VirtualDJ that warrants further investigation on a larger sample.
...
VirtualDJ cross-platform analysis (Table~\ref{tab:vdj-rb}) confirms these findings. Matching 22 tracks between Rekordbox and VirtualDJ by file path and duration confirms identical audio files. Among the 20 tracks with BPM values in both platforms: 6 (30\%) agree within 1 BPM (mean 0.15 BPM difference); 5 tracks (25\%) from the ETHEREAL AFTERHOURS playlist exhibit a consistent 1.19$\times$ ratio (Rekordbox stores 140.0--141.4 BPM, VirtualDJ stores 118.0--119.0 BPM), a distinct pattern from the Rekordbox 2$\times$ convention that suggests algorithmic half-tempo interpretation rather than a storage error; the remaining 9 tracks (45\%) show unexplained BPM disagreements without a clean ratio, concentrated in tracks at the 140--175 BPM stored range. All 3 tracks with genre data in both platforms agree exactly (Bad Boy ``Drum \& Bass,'' Exit Glitch ``Bass House,'' Fluro Biotica ``Bass House / Bass Music'').
...
\begin{table}[t]
  ...
  \begin{tabular}{lllc}
    ...
    \bottomrule
  \end{tabular}
  \caption{BPM comparison across 16 representative tracks from 20 matched Rekordbox--VirtualDJ tracks (partial library scan, n=22 matched tracks; illustrative, not representative). ``Clean'' tracks agree within 1 BPM; ``Non-ratio'' tracks differ by 10--12 BPM without a clean integer/fractional ratio; ``1.19$\times$ ratio'' tracks suggest algorithmic half-tempo interpretation disagreement. Two tracks (Bad Boy, Neuroresonance) had no VirtualDJ BPM value and are omitted.}
  \label{tab:vdj-rb}
\end{table}
...
\textbf{Limitations of the maintenance approach.} Cross-platform voting requires access to multiple platform exports, which not all DJs maintain. The 2$\times$ interpretation difference detection assumes a second platform stores the cross-platform reference BPM, which is validated here for Serato and Engine DJ but may not hold for all platform pairs. Genre maintenance requires either manual annotation or audio-based classification, as external databases (MusicBrainz) have insufficient EDM coverage. These constraints define the design space for a production UDMS maintenance system.
