...
\begin{itemize}
  ...
  \item The first cross-platform DJ metadata interoperability analysis: using 143 tracks matched between Rekordbox and Serato, we quantify preservation rates across all UDMS fields, characterize a 2$\times$ half-tempo BPM interpretation difference in Rekordbox that affects a substantial fraction of half-tempo tracks, and present preliminary evidence from 24 matched Rekordbox--VirtualDJ tracks (20 with BPM in both) suggesting a distinct 1.19$\times$ algorithmic half-tempo interpretation pattern in VirtualDJ that warrants further investigation on a larger sample.
...
Of the 24 tracks appearing in both Rekordbox and VirtualDJ (matched by artist and title), 20 have BPM values in both platforms, enabling direct tempo comparison. The disagreements fall into two distinct clusters:
