...
Our contributions:
\begin{itemize}
  \item UDMS, a canonical schema normalizing Rekordbox, Serato, Engine DJ, and Traktor metadata fields into a unified representation (11 fields, open-source Python implementation).
  \item The first empirical metadata quality analysis of a DJ library: field coverage, completeness tiers, genre taxonomy fragmentation, and key distribution across three platforms.
  \item The first cross-platform DJ metadata interoperability analysis: using 143 tracks matched between Rekordbox and Serato, we quantify preservation rates across all UDMS fields, identify a Rekordbox 2$\times$ BPM bug affecting 27\% of tracks, and validate it against a third platform (Engine DJ).
...
We analyzed a Rekordbox~7.2.8 (AlphaTheta) XML export containing 636 tracks, a Serato~2.0 database export containing 382 tracks, and an Engine DJ (Engine Prime) SQLite database containing 112 tracks from the same DJ's library. Of these, 143 tracks appear in both Rekordbox and Serato, enabling cross-platform interoperability analysis. The library spans multiple acquisition sources: DJ playlists (42.3\%), local downloads (24.1\%), monthly download batches (18.4\%), SoulSeek (13.1\%), Rekordbox samplers (1.9\%), and Pioneer DJ demos (0.3\%). File formats: AIFF (71.4\%), FLAC (18.1\%), MP3 (8.6\%), WAV (1.9\%).
...
This study analyzes libraries from one DJ across three platforms (Rekordbox, Serato, and Engine DJ). The Rekordbox and Serato libraries share the same DJ (143 track overlap), while the Engine DJ library is from a separate DJ with no overlap, limiting direct cross-platform matching. Library composition reflects the DJ's personal taste (electronic/bass music focus), limiting generalizability. The 636-track Rekordbox dataset and 382-track Serato dataset, while substantial, are smaller than typical commercial DJ libraries. The cross-platform analysis is limited to 143 matched tracks between Rekordbox and Serato; the unmatched tracks (493 Rekordbox-only, 239 Serato-only) may have different quality profiles. The Engine DJ analysis covers 112 tracks with 100\% BPM (via bpmAnalyzed) and key coverage, providing a useful third-platform comparison for field completeness analysis. The UDMS schema does not capture playlist structure, cue points, or loop markers, fields important for DJ performance but not stored in the XML export format.
...
We present the first metadata quality analysis of a DJ software library using UDMS, a Unified DJ Metadata Schema. Our analysis of 636 Rekordbox tracks, 382 Serato tracks, and 112 Engine DJ tracks reveals that while audio-derived fields and musical key achieve high coverage (93--100\%), genre annotation, the field most critical for automated playlist generation, is incomplete for 37--55\% of tracks across platforms. Cross-platform interoperability analysis across 143 matched tracks between Rekordbox and Serato shows that BPM and key are reliably preserved (89.5\% and 100\% effective agreement), but genre agreement is only 79.1\% even when both platforms are tagged. A previously undocumented Rekordbox 2$\times$ BPM bug affects 27\% of tracks; we validate this bug against a third platform (Engine DJ), which correctly stores the reference tempo. Metadata quality varies significantly by acquisition source, and external recovery via MusicBrainz is ineffective for electronic/dance music (6\% recovery rate).
...
\begin{itemize}
  ...
  \item \textbf{Dataset:} The 636-track Rekordbox XML export, 382-track Serato database export, and 112-track Engine DJ SQLite database (anonymized) are included in the repository at \path{data/}. Track filenames and paths are anonymized; metadata fields are preserved. The cross-platform matched track dataset is at \path{data/cross_platform_comparison.json}; the Engine DJ dataset is at \path{data/engine_dj_tracks.csv}.
